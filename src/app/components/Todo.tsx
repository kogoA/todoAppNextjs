...
const Todo = ({ todo }: TodoProps) => {
    ...
    return (
        <li key={todo.id} className="flex justify-between p-4 bg-white border-l-4 border-blue-500 rounded shadow">
            {isEditing ? (<input type="text" value={editedTaskTitle} onChange={(e: React.ChangeEvent<HTMLInputElement>) => seteditedTaskTitle(e.target.value)} className="mr-2 py-1 px-2 rounded border-gray-400 border" />):
            (<span>{todo.text}</span>) }
            <div>
                {isEditing ? 
